              ...
              {isOpen && (
                <div className="border-t border-slate-100 bg-[#fbfdf9] px-4 pb-6 pt-5 dark:border-slate-800 dark:bg-[#09170f] sm:px-7">
                  ...
                  <div className="mt-5 flex flex-col gap-4 border-t border-slate-200 pt-5 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <Link
                      href={`/payment-declaration?planId=${encodeURIComponent(plan.id)}&plan=${encodeURIComponent(plan.name)}&price=${encodeURIComponent(plan.price)}&family=${encodeURIComponent(plan.family)}`}
                      className="flex w-full items-center justify-center rounded-full bg-[#064627] px-7 py-3 text-sm font-black text-white transition hover:bg-[#095936] sm:w-auto"
                    >
                      Choose {plan.name}
                    </Link>
                  </div>
